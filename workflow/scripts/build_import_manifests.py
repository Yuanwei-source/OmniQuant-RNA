...
def main():
    ...
    salmon = build_manifest(
        ...
        import_type="salmon",
        file_role="quant.sf",
        resolver=lambda sample: f"results/05.quantification/native/salmon/per_sample/{sample}/quant.sf",
    )
    kallisto = build_manifest(
        ...
        resolver=lambda sample: f"results/05.quantification/native/kallisto/per_sample/{sample}/abundance.tsv",
    )
    stringtie = build_manifest(
        ...
        resolver=lambda sample: f"results/05.quantification/native/stringtie/per_sample/{sample}/final/t_data.ctab",
    )
