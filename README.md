...
- 模块化运行：支持完整流程和按模块执行。
- 日志与结果分层：正式输出、原始矩阵、质控快照和共识诊断分开组织。
...
- [results/04.quantification](results/04.quantification)：各定量器原始输出。
- [results/tables/raw_matrices](results/tables/raw_matrices)：汇总表达矩阵。
- [results/tables/qc_snapshots](results/tables/qc_snapshots)：如 StringTie gene_id 映射等质控快照。
