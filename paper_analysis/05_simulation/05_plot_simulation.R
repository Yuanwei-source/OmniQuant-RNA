...
pA <- ggplot(clean %>% filter(!Method %in% c("CCT","RRA")), 
  ...
  labs(title="A. Clean benchmark: F1 score",y="F1",x="") +
  scale_x_discrete(labels=method_labels) + theme_minimal(base_size=11) +
...
pB <- ggplot(stress %>% filter(!is.na(precision_mean)), 
  ...
  labs(title="B. Stress: Precision",y="Precision",x="") +
  scale_x_discrete(labels=method_labels) + theme_minimal(base_size=11) +
...
pC <- ggplot(stress %>% filter(!is.na(iso_fp_rate_mean)), 
  ...
  labs(title="C. Stress: Isoform-switch FP%",y="Isoform-switch false positive rate (%)",x="") +
  scale_x_discrete(labels=method_labels) + theme_minimal(base_size=11) +
...
pD <- ggplot(stress %>% filter(!is.na(precision_mean)), 
  ...
  labs(title="D. Stress: Precision-Recall trade-off",x="Recall",y="Precision") +
...
if ("seed" %in% names(stress_all)) {
  ...
  pE <- ggplot(seed_long, aes(x=Method, y=iso_fp*100, group=seed, color=factor(seed))) +
    geom_line(alpha=0.6) + geom_point(size=2) +
    labs(title="E. Per-seed: Iso-switch FP reduction",y="Iso-switch FP%",x="") +
    scale_color_discrete(name="Seed") + theme_minimal(base_size=11) +
...
combined <- pA + pB + pC + pD + pE + plot_layout(design=design, guides="collect") +
  plot_annotation(title="Figure X. Consensus performance under clean and isoform-switching stress simulations",
